...
#both work, depends on ordering of variables you need
```
...
# minus drops just the variable names you want to get rid of
```
...
# everything is everything you havent specified about yet
```
...
# rename variable and pull everything up

rename(gapminder, cont = continent)
# just rename variable

rename_all(gapminder, ~toupper(.x))
# how to rename all variables at once, the tilda says do what im about to say to everything
```

Playing with select functions in mtcars
```{r}
as_tibble(mtcars)
select(mtcars, drat: gear)
select(mtcars, -cyl)

select(mtcars, gear, cyl, mpg, everything())
...
#arrange by the year in descending order
arrange(gapminder, desc(year))
arrange(gapminder, -year)

#arrange by the year in ascending order
arrange(gapminder, year)
...
%in% goes in between to functions
...
# when you use the then mark you don't need to input the first argument because it assumes it 
gapminder %>%  
...
```{r}
?if_else
# make sure not to use a double equal sign for variable = if_else
gapminder %>%
  mutate(lifeExp = if_else(country == "Canada" & year == 1952, 
                            70, 
                            lifeExp))
# NA_real shows that the missing value is a number not a logical character
gapminder %>% 
