...
1. Would you say this data is in tidy format?
   ...
   I would say that it is tidy because each row is one observation and the order makes it simple to make easy to make comparisons of how many words different races of different gender are saying in various movings. Depending on the analyses you wanted to do you might consider changing the order based on movie instead of gender but you could also just use filter = movie if you wanted to limit comparisons to a specific movie. 
...
```{r}
(guest %>% 
  pivot_longer(cols      = c(meal_brunch, meal_wedding), 
               names_to  = "meals",
               values_to = "n_meals")) %>% 
  pivot_longer(cols = c(attendance_golf, attendance_wedding, attendance_brunch), 
               names_to = "attendance", 
               values_to = "n_attendance") -> guest_long 
  print(guest_long)
...
```
None of the parties have a pending status for all memebers and all events.
...
```
Parties 3, 4, 8, and 10 have a pending status for all of their party members for the wedding.
...
```{r}
guest_long %>% 
  pivot_wider(id_cols = c(-meals, -n_meals), 
              names_from = meals, 
              values_from = n_meals) %>% 
  pivot_wider(id_cols = c(-attendance, -n_attendance), 
              names_from = attendance, 
              values_from = n_attendance) %>% 
  print(guest_long)
...
```{r, warning = FALSE}
email %>% 
  separate(guest, into = c("first", "last")) %>% 
  unite(col = "guest", first:last, sep = " ")
```
