...
as_tibble(mtcars ) %>% 
  filter(cyl == 6) %>% 
  select("Model", mpg, hp) -> sixcylsubset
...
rename(sepal_length = sepal.length, sepal_width = sepal.width, petal_length = petal.length, petal_width = petal.width) %>% 
  select(species, everything()) -> newiris
...
newiris %>% 
  ...
  xlab("Species") +
  ylab("Sepal Width") +
  geom_jitter(aes(color = species, alpha = .7))
...
# instead of using the tilda you could use vars(species)
```
...
```{r}
# if you want mins for all use mutate, if you want 1 value use summarise
gapminder %>% 
...
rowise tells r to compute based on each row
```{r}
psych::bfi %>%
  as_tibble() %>% 
  select(A1:A5) %>% 
  ...
  mutate(A_sd = sd(c(A1, A2, A3, A4, A5, na.rm = TRUE))) %>% 
  ungroup()
```
...
psych::bfi %>%
  ...
  ungroup() %>% 
  group_by(C_mean, C_sd) %>% 
  summarise(C_min = min(c(C1, C2, C3, C4, C5), na.rm = TRUE)) %>% 
  select(C_mean, C_sd, C_min)
```
...
psych::bfi %>% 
  ...
  select(education, O_mean, C_mean, E_mean, A_mean, N_mean) -> newpsych
newpsych
```
